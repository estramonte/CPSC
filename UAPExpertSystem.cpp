...
/\/\/\/\/\/\/\/\/\/\/\/\/\ //
      ...
      // The conditional expression for the while loop must involve the Boolean
//variable bUserWishesToContinue,
      // defined above:
// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
/\/\/\/\/\/\/\/\/\/\/\/\/\ //
    ...
    while (bUserWishesToContinue)
      {
            ...
            std::cout << "For all questions, please answer 1 for Yes and 0 for No."
<< std::endl;;
            std::cout << std::endl;
...
//////////////////////////////
            ...
            // Unknown Aerial Phenomenon (UAP) Characteristics (only if visually
            //sighted or photographed) Decision Variables
...
/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
            ...
            // The vector should have elements of the std::string data type. The
//declaration will have the following form where you will replace with question
//marks (????) with the remainder of the declaration.
...
/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
            ...
            if (bUapDetectedVisuallyOrPhotographed)
            {
                ...
                if (bUapAbruptManeuvers)
                {
                    ...
                    {
                          bUapDetectedByRadar ?
vecReason.push_back(strReasonUapDetectedByRadar) : vecReason.push_back(strPrefixNo
+ strReasonUapDetectedByRadar);
                          bUapDetectedByIR ?
vecReason.push_back(strReasonDetectedByIR) : vecReason.push_back(strPrefixNo +
...
bUapDiscernableMeansOfPropulsion)
                          {
                              if (!bUapConrolSurfacesVisible)
                              {
                                  vecReason.push_back(strPrefixNo +
strReasonControlSurfacesVisable);
                              }
                              if (!bUapDiscernableMeansOfPropulsion)
                              {
                                  vecReason.push_back(strPrefixNo +
strReasonUapDiscernableMeansOfPropulsion);
                              }
                              // Rationale
                          ...
                          {
                              bUapConrolSurfacesVisible ?
vecReason.push_back(strReasonControlSurfacesVisable) :
vecReason.push_back(strPrefixNo + strReasonControlSurfacesVisable);
                              bUapDiscernableMeansOfPropulsion ?
vecReason.push_back(strReasonUapDiscernableMeansOfPropulsion) :
vecReason.push_back(strPrefixNo + strReasonUapDiscernableMeansOfPropulsion);
                              ...
                              strExplanatoryCategory =
strExplanatoryCategoryAdvancedSystem;
                              bCategorized = true;
...
strReasonUapDetectedByRadar);
                          vecReason.push_back(strPrefixNo +
strReasonDetectedByIR);
                          if (bUapObservedToUndulateOrChangeShape &&
bBirdsInVicinity)
                          {
      ...
      vecReason.push_back(strReasonUapObservedToUndulateOrChangeShape);
                    ...
                    // Flocks of birds can show up on radar and IR
                    //and the mass appears to change shape or undulate when viewed as a whole.
                    // Birds don't always show on radar though if
                    //flock is small.
                ...
                else if (bRecreationalUavActivityFeasible)
                {
                    ...
                    bUapObservedToUndulateOrChangeShape ?
vecReason.push_back(strReasonUapObservedToUndulateOrChangeShape) :
vecReason.push_back(strPrefixNo + strReasonUapObservedToUndulateOrChangeShape);
                    bBirdsInVicinity ?
vecReason.push_back(strReasonBirdsInVicinity) : vecReason.push_back(strPrefixNo +
strReasonBirdsInVicinity);
...
bUapDiscernableMeansOfPropulsion)
                  ...
                  {
                      vecReason.push_back(strPrefixNo +
                        strReasonDetectedVisuallyOrPhotographed);
...
// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
            // TODO: IN THE FOLLOWING IF BLOCK, ENTER THE BLOCK IF THE
            //UAP WAS BOTH DETECTED BY RADAR
...
/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
            ...
            if (bUapDetectedByRadar && bUapDetectedByIR)
            {
                vecReason.push_back("UAP was detected by radar");
                vecReason.push_back("UAP was detected by infra-red");
              ...
              else if (bUapUnusualFlightCharacteristics)
              {
                  ...
                  if (bUapDetectedByRadar && bUapDetectedByIR)
                  {
                      ...
                      // Rationale
                        ...
                        {
                            vecReason.push_back(strPrefixNo +
                              strReasonUapSignatureManagement);
                              if (bRecreationalUavActivityFeasible)
                              {
                                ...
                                {
                                    vecReason.push_back(strPrefixNo +
                                      strReasonRecreationalUavActivityFeasible);
                                    vecReason.push_back(strReasonBirdsInVicinity);
...
// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
// TODO: ITERATE OVER THE vecReason VECTOR AND EMIT STRINGS TO STANDARD
//OUTPUT IN ORDER

// Use a for loop; use the vector size() function and the vector at()
//function as well as std::cout.
...
// Prompt user using std::cout; collect input from user using std::cin
//into char variable cUserResponse;

// allow user to type either 'Y' or 'y' to continue. Any other
//character is considered 'n'.

// Use an OR operator (or, ||) and the NOT operator (!) in your
//solution.

// ITERATE AGAIN THROUGH WHILE LOOP OR TERMINATE PROGRAM BASED ON USER
//RESPONSE

// Alter value of variable bUserWishesToContinue to terminate while
//loop if user response requires.
